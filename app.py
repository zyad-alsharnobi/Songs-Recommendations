...
def recommend_songs(data, mood_features, num_recommendations=5):
    ...
    # Extract only the relevant features from mood_features
    relevant_features = ['danceability', 'energy', 'loudness','acousticness','instrumentalness','valence','tempo']
    mood_vector = np.array([mood_features[feature] for feature in relevant_features])
    
    # Extract the relevant features from the dataset
    features = data_copy[relevant_features].values
